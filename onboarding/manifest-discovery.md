...
`fixtures.discovery_proof` 是 `table`、`formula`、`supplementary` 的强制候选池证明。它不是叙述性备注；`validate-manifest` 会机器校验它。
...
`doi: null` 的 `table`、`formula`、`supplementary` 必须有 `exhausted: true`、至少 3 条 query、候选拒绝理由，且 `evidence_reason` / `evidence_summary` 不能只写“未找到样本”。如果同一 manifest 的 non-null fixture 或 cleaning evidence 暴露了该 purpose 的强信号，worker 必须选择该 DOI，或在 `rejections` 中用同 DOI 明确说明为什么它不适合作为该 purpose fixture。
...
- `fixtures.discovery_proof` contains `table`, `formula`, and `supplementary`.
