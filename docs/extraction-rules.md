...
- Front matter 的 publication watermark 只匹配短 masthead 标签：必须是短文本、无句子标点、token 数受限，并且呈标题式或全大写；`science` / `pnas` / `ams` / `bams` 等 provider 词面只作为 provider-scoped keyword 参与判断，长正文句子里出现这些词不能被当作 front matter。
...
### Royal Society Silverchair figure caption 和 PDF fallback 结构必须保真

- 这条规则约束的是：Royal Society Publishing 的 Silverchair HTML 图像资产必须从 `div.fig-section` 读取真实 figure label、caption 和 `/view-large/figure/` 链接，不能把正文图降级成文末 `- Figure` 占位；PDF fallback Markdown 必须从 PDF front matter 恢复真实标题、作者、摘要和参考文献，并移除 citation/sidebar/license 前置信息、Royal Society 下载水印、行内水印代码片段、空代码 fence、页码标记和 `picture [...] intentionally omitted` 占位符。
- 如果违反，用户会看到：`## Figures` 下只有多个 `- Figure`，或者 PDF fallback 以 DOI 当标题、缺少 `## Abstract`/`## References`、正文夹杂 `Cite this article`/`Subject Areas`/`Downloaded from http://royalsocietypublishing.org`/机构用户水印/删除图片提示/页码噪声。
- 它对应的阶段是：`asset-discovery`、`markdown-normalization`、`final-rendering`。
- Owner：`paper_fetch.providers._royalsocietypublishing_html`、`paper_fetch.providers.royalsocietypublishing.prepare_pdf_fallback_markdown` 与 `paper_fetch.providers.royalsocietypublishing.clean_pdf_markdown`。
- 代表性 HTML / PDF：
  ...
  - [`../tests/fixtures/golden_criteria/10.1098_rsta.2020.0108/original.pdf`](../tests/fixtures/golden_criteria/10.1098_rsta.2020.0108/original.pdf)
- 对应测试：
  ...
  - [`../tests/unit/test_royalsocietypublishing_provider.py`](../tests/unit/test_royalsocietypublishing_provider.py) 中的 `test_markdown_contract_pdf_fallback_fixture`
  - [`../tests/unit/test_royalsocietypublishing_provider.py`](../tests/unit/test_royalsocietypublishing_provider.py) 中的 `test_pdf_markdown_cleanup_removes_royal_society_watermark_and_placeholders`
- 边界说明：
  - 这条规则只针对 Royal Society Publishing/Silverchair 的 HTML figure wrapper 和 PDF fallback 水印形态；不会改变其他 provider 的通用 PDF Markdown 转换策略。
