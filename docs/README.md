...
- `wiley` / `science` / `pnas` / `ams` 的 challenge 恢复链路只接受能识别为图片的 FlareSolverr `solution.imagePayload`，包括浏览器导出的 PNG 和原始 SVG；不会再把图片文档 screenshot 裁剪成正文图片资产，也不会把 challenge HTML 保存成图片。
