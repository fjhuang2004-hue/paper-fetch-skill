...
- browser workflow 的 bootstrap、PDF/ePDF fallback、article assembly、asset retry helper、client 基类和 Playwright fetchers 已收敛到 `browser_workflow/` 子包；profile 只面向 provider catalog 中的 `science` / `pnas` / `wiley` / `ams`。
...
- `ams`
  ...
  - 已回填正文的 AMS figure / table 下载后会改写为本地图片链接，并从尾部 `Figures` / `Tables` 附录去重。
...
## Elsevier / Springer / Wiley / Science / PNAS / AMS / IEEE / arXiv / Copernicus 的特殊语义
...
- `wiley` / `science` / `pnas` / `ams` 的 `asset_profile=all` 会把 supplementary 作为独立文件附件下载。
...
- `wiley` / `science` / `pnas` / `ams` 正文图片主链路只输出 `download_tier="full_size"` 或 `download_tier="preview"`。
- supplementary 文件链路输出 `download_tier="supplementary_file"`。
- 旧的 `playwright_canvas_fallback` tier 只可能来自仍保留 HTTP-first 语义的旧通用图片下载路径。
- `wiley` / `science` / `pnas` / `ams` 正文图片下载会缓存重复的 figure page / 图片候选 URL。
