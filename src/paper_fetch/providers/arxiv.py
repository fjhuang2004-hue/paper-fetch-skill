...
class ArxivClient(ProviderClient):
    ...
    def __init__(
        ...
        self.api_client = api_client or arxiv_api.Client(
            page_size=1,
            delay_seconds=ARXIV_API_DELAY_SECONDS,
            num_retries=ARXIV_API_NUM_RETRIES,
        )
        session = getattr(self.api_client, "_session", None)
    ...
    def fetch_metadata(self, query: Mapping[str, str | None]) -> ProviderMetadata:
        ...
        try:
            search = arxiv_api.Search(id_list=[arxiv_id], max_results=1)
