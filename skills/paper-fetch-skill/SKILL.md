...
- 如果目标论文属于 `science`、`pnas`、`wiley`、`ams` 这类依赖浏览器运行时的 provider，在第一次抓取前先启动 FlareSolverr，并用 `provider_status()` 或仓库现成的状态脚本确认运行时健康后再抓取。
- 如果 `wiley`、`science`、`pnas`、`ams` 这类依赖浏览器运行时的抓取首次失败，可以在排除明显配置错误后最多再重试 `2` 次；重试时优先绕过缓存，并确认本地浏览器运行时或 FlareSolverr 健康状态。若仍失败，要明确告诉用户失败发生在浏览器链路。
- `ieee` 不需要 FlareSolverr 或 IEEE API key；它会默认尝试 Xplore dynamic HTML，再回退到 text-only PDF。无合法访问上下文、验证码、登录页或非 PDF payload 都应被视为正常降级原因，而不是重试浏览器链路。
...
10. 如果目标 provider 是 `wiley`、`science`、`pnas` 或 `ams`，在第一次抓取前先启动 FlareSolverr；随后调用 `provider_status()` 或仓库现成的状态脚本确认本地浏览器运行时健康。
11. 如果提供方凭证、Wiley / Science / PNAS / AMS 的本地运行时状态，或 IEEE Xplore 访问上下文可能影响结果，在第一次抓取前调用 `provider_status()`。
